    ...
    // remove datas default from HTML
        ...
        if(products.length > 0) // if has data
        {
            products.forEach(product => {
                ...
                <h2 class="title card-title">${product.name}</h2>
                <h2 class="card-text">${product.description}</h2>
                <h2 class="card-text">${product.category}</h2>
               <div class="price card-text mb-5">${product.price}$</div>
...
const addCartToMemory = () => {
    localStorage.setItem('wall-cart', JSON.stringify(cart));
}
const addCartToHTML = () => {
...
const initApp = () => {
    ...
    .then(data => {
        ...
        if(localStorage.getItem('wall-cart')){
            cart = JSON.parse(localStorage.getItem('wall-cart'));
            addCartToHTML();
        }
    })
}
