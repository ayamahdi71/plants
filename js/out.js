    ...
    // remove datas default from HTML
        ...
        if(products.length > 0) // if has data
        {
            products.forEach(product => {
                ...
                <button class="btn btn-pay btn addCart">Add To Cart</button>`;
...
const addCartToMemory = () => {
    localStorage.setItem('out-cart', JSON.stringify(cart));
}
const addCartToHTML = () => {
...
const initApp = () => {
    ...
    .then(data => {
        ...
        if(localStorage.getItem('out-cart')){
            cart = JSON.parse(localStorage.getItem('out-cart'));
            addCartToHTML();
        }
    })
}
